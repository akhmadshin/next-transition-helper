...
const y = (s) => () => {
  ...
  const t = `${o}-${window.__NTH_previousRouterKey}`, w = d(t), _ = sessionStorage.getItem(`__NTH_view_transition_image_selector_${t}`), n = _ ? document.querySelector(_) : void 0;
  if (n && w) {
    ...
    if (e) {
      const a = u(e) || "", r = document.querySelector("[style*='view-transition-name']");
      r && (r.style.viewTransitionName = ""), e.style.viewTransitionName = "__NTH_transition-img", sessionStorage.setItem(`__NTH_view_transition_image_selector_${t}`, a);
    }
  }
  window.__NTH_transitionImgSrc = void 0, window.__NTH_pageMounted && (window.__NTH_pageMounted(), window.__NTH_pageMounted = void 0);
};
export {
  y as getHandleRouteChangeComplete
};
